...
public partial class Panel
{
	...
	private bool LoadStyleSheetFromAttribute()
	{
		...
		if ( attr == null )
		{
			if ( !string.IsNullOrWhiteSpace( loadedStyleSheetPath ) )
				StyleSheet.Remove( loadedStyleSheetPath );

			loadedStyleSheetPath = null;
			return false;
		}
	...
	private bool LoadStyleSheetFromPath( string path, bool failSilently )
	{
		...
		// Add new one
		loadedStyleSheetPath = path;
		StyleSheet.Load( loadedStyleSheetPath, true, failSilently );
	...
	/// <summary>
	/// Called when a child created from razor markup has a <c>slot</c> attribute. Walks up the
	/// hierarchy until a panel claims the slot.
	/// </summary>
